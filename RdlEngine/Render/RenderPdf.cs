...
namespace fyiReporting.RDL
{
    ...
    internal class RenderPdf : IPresent
    {
        ...
        {
            // Get the fill color
            cb.SetRGBColorFill(c.R, c.G, c.B, c.A);
        ...
        {
            if (si.BackgroundColor == Color.Transparent)
        ...
        private void iAddRectangle(float x, float y, float height, float width, StyleInfo si, string url, PdfPattern patterns, string tooltip)
        {
            ...
            if (si.BackgroundColor != Color.Transparent && height > 0 && width > 0)
        ...
        private string FontFolder {
            get 
            {
                ...
                {
                    var _fontsDir = Path.Combine(Android.OS.Environment.RootDirectory.AbsolutePath, "fonts");
                    if (System.IO.Directory.Exists(_fontsDir))
                    {
                        return _fontsDir;
                    }
         ...
         return System.Environment.GetFolderPath(System.Environment.SpecialFolder.Fonts);
                ...
                return System.IO.Path.Combine (dirWindowsFolder.FullName, "Fonts");
                // Results in full path e.g. "C:\Windows\Fonts" 
#endif
            }
        }

        public PdfImages images
        {
            get { return m_Images; }
            set { m_Images = value; }
        }
            ...
            StyleInfo si, PdfFonts fonts, float[] tw, bool bWrap, string url, bool bNoClip, string tooltip)
        {
            ...
            if (face == "Times-Roman")
            {
                ...
                if (face == "Arial")
                {
                    if (si.IsFontBold() && si.FontStyle == FontStyleEnum.Italic)
                    {
                        face = dejavuFonts ? "DejaVu Sans Condensed Bold Oblique" : "Roboto-BoldItalic";
                        fontname = (dejavuFonts ? "DejaVuSansCondensed-BoldOblique.ttf" : "Roboto-BoldItalic.ttf");
                    ...
                    {
                        face = dejavuFonts ? "DejaVu Sans Condensed Bold" : "Roboto-Bold";
                        fontname = (dejavuFonts ? "DejaVuSansCondensed-Bold.ttf" : "Roboto-Bold.ttf");
                    ...
                    {
                        face = dejavuFonts ? "DejaVu Sans Condensed Oblique" : "Boboto-Italic";
                        fontname = (dejavuFonts ? "DejaVuSansCondensed-Oblique.ttf" : "Roboto-Italic.ttf");
                    ...
                    {
                        face = dejavuFonts ? "DejaVu Sans Condensed" : "Roboto-Regular";
                        fontname = (dejavuFonts ? "DejaVuSansCondensed.ttf" : "Roboto-Regular.ttf");
            ...
            for (int i = 0; i < sa.Length; i++)
            {
                ...
                if (si.BackgroundColor != Color.Empty)
                {
                    if (si.BackgroundColor != Color.Transparent && height > 0 && width > 0 && i == 0)
                    {
                        // background color, height and width are specified
                ...
                if (height > 0 && width > 0)
                {
                    cb.SetRGBColorFill(si.Color.R, si.Color.G, si.Color.B, si.Color.A);
        ...
        private void ProcessPage(Pages pgs, IEnumerable items)
        {
            foreach (PageItem pi in items)
            {
                ...
                {	// put out any background image
                    ...
                    //float imW = Measurement.PixelsFromPoints(bgImg.SamplesW, pgs.G.DpiX);
                    //float imH = Measurement.PixelsFromPoints(bgImg.SamplesH, pgs.G.DpiY);
                    float imW = bgImg.SamplesW;
                    float imH = bgImg.SamplesH;
                    int repeatX = 0;
                    int repeatY = 0;
                ...
                if (pi is PageImage)
                {
                    ...
                    switch (i.Sizing)
                    {
                        ...
                        case ImageSizingEnum.Clip:
                            //adjustedRect = new RectangleF(r2.Left, r2.Top, Measurement.PixelsFromPoints(i.SamplesW, pgs.G.DpiX), Measurement.PixelsFromPoints(i.SamplesH, pgs.G.DpiY));
                            adjustedRect = new RectangleF(r2.Left, r2.Top, i.SamplesW, i.SamplesH);
                    ...
                    //if (i.ImgFormat == ImageFormatType.Wmf || i.ImgFormat == ImageFormatType.Emf)
                    if (i.ImgFormat == ImageFormat.Jpeg)
                    {
        ...
        private string[] MeasureString(PageText pt, Graphics g, out float[] width)
        {
            ...
            try
            {
                ...
                if (pt.NoClip || pt.SI.WritingMode == WritingModeEnum.tb_rl)	// TODO: support multiple lines for vertical text
                {
                    ...
                    width = new float[1];
                    //width[0] = Measurement.PixelsFromPoints(ms.Width, g.DpiX);	// convert to points from pixels
                    width[0] = ms.Width;
                    sa = new string[1];
                    sa[0] = s;
                    return sa;
                ...
                foreach (string tfl in flines)
                {
                    ...
                    float tw = ms.Width;
                    //float tw = Measurement.PixelsFromPoints(ms.Width, g.DpiX);
                    if (tw <= ptWidth)
        ...
        private WordStartFinish[] MeasureString32(string s, Graphics g, System.Drawing.Font drawFont, StringFormat drawFormat, CharacterRange[] cra)
        {
            ...
            foreach (Region r in rs)
            {
                ...
                //sz[isz].start = Measurement.PixelsFromPoints(mr.Left, g.DpiX);
                //sz[isz].end = Measurement.PixelsFromPoints(mr.Right, g.DpiX);
                sz[isz].start = mr.Left;
                sz[isz].end = mr.Right;
                isz++;
            }
            return sz;
        ...
        private SizeF MeasureString(string s, Graphics g, System.Drawing.Font drawFont, StringFormat drawFormat)
        {
            ...
            //return g.MeasureString(s, drawFont, 0, drawFormat);
            /////// final substituição
        }
        ...
        private float MeasureStringBlank(Graphics g, System.Drawing.Font drawFont, StringFormat drawFormat)
        {
            ...
            //float width = Measurement.PixelsFromPoints(ms.Width, g.DpiX);	// convert to points from pixels
            float width = ms.Width;
            return width * 2;
        }
