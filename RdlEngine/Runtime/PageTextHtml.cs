	...
	public class PageTextHtml : PageText, IEnumerable, ICloneable
	{
		...
		private SizeF MeasureString(string s, StyleInfo si, Graphics g, out float descent)
		{
			...
			try
			{
			    ...
			    var _size = g.MeasureString(s, drawFont, 0, drawFormat);
                ms.Height = RSize.PointsFromPixels(g, _size.Height);	// convert to points from pixels
                ms.Width = RSize.PointsFromPixels(g, _size.Width);		// convert to points from pixels
